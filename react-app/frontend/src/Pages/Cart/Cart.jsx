...
const ShoppingCart = () => {
  ...
  const BACKEND_URL = "http://localhost:8080/api/user/cart/getItems";
    ...
    const fetchCartItems = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `http://localhost:8080/api/user/cart/getItems`,
          {
            headers: {
  ...
  const removeItem = async (productIdToRemove) => {
    try {
      ...
      const response = await axios.delete(
        `http://localhost:8080/api/users/cart/remove/${currentUserId}/${productIdToRemove}`,
        {
          headers: {
            Authorization: `Bearer ${token}`, // Include token for authentication
  ...
  const handleConfirmPurchase = async () => {
    ...
    try {
      ...
      if (orderResponse.status === 200 || orderResponse.status === 201) {
        ...
        await axios.delete(
          `http://localhost:8080/api/users/cart/remove/${currentUserId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`, // Include token for authentication
  ...
  const handleSaveNewAddress = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:8080/api/users/addresses/add/${currentUserId}`,
        addressPayload,
        {
          headers: {
            Authorization: `Bearer ${token}`, // Include token for authentication
          ...
          {cartItems.map((item) => (
            <div key={item.productId} className="shoppingcart-item">
              ...
              <div className="shoppingcart-item-details">
                <h3>{item.title}</h3>
                {/* Update: Show total price for this item */}
                <p>
